...
(() => {
 ...
 panels.time.append(fields[1]);fields[1].querySelector('legend').textContent='Time spent today';fields[1].querySelector('p').textContent='Count each activity once. Your total must fit within 24 hours.';
 ...
 [...form.querySelectorAll('#timeInputs input,#stageInputs input'),$('#time_sleep')].forEach(meter);
 ...
 form.addEventListener('invalid',e=>{const panel=e.target.closest('.entry-panel');if(panel)choose(panel.id.replace('panel-',''));const detail=e.target.closest('details');if(detail)detail.open=true;},true);
